...
class RememberMeForm(AuthenticationForm):
    remember_me = forms.BooleanField(required = False, initial = True, widget=forms.CheckboxInput(attrs={'class':'login__check-input'}))
...
class RegisterForm(UserCreationForm):
    ...
    last_name = forms.CharField(label="", max_length=100, widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'Segundo Nome'}))
...
class TrainingPlanning(forms.ModelForm):
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        if user and isinstance(user, User):
            self.fields['aparelho'].queryset = Aparelho.objects.filter(users=user)
